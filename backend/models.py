...
class Event(Base):
    ...
    status: Mapped[str] = mapped_column(SAEnum("pending", "booked", "cancelled", "ausverkauft", "teilweise_ausverkauft", name="event_status"), default="pending", nullable=False)
    capacity: Mapped[int] = mapped_column(Integer, nullable=True)
    source: Mapped[str] = mapped_column(SAEnum("scraper", "manual", name="event_source"), default="manual", nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=lambda: datetime.now(timezone.utc), nullable=False)
    force_keep: Mapped[bool] = mapped_column(default=False, nullable=False)
    force_keep_note: Mapped[str | None] = mapped_column(String(500), nullable=True)
...
class RSVP(Base):
    ...
    companions: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    selected: Mapped[bool] = mapped_column(default=False, nullable=False)
    reminder_sent_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    token: Mapped[str] = mapped_column(String(64), unique=True, default=lambda: secrets.token_urlsafe(32), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=lambda: datetime.now(timezone.utc), nullable=False)
