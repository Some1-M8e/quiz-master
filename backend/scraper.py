...
def scrape_provider(provider: Provider, db: Session) -> list[dict]:
    ...
    for event_el in soup.find_all(class_=lambda c: c and "eventlist-event--upcoming" in c):
        ...
        # "pending" = buchbar mit 4+ Slots wird sofort gebucht
        # "teilweise_ausverkauft" = 3 oder weniger Plätze → wird NICHT gebucht
        if any(word in status for word in ["abgesagt", "cancelled"]):
            ...
            event_status = "teilweise_ausverkauft"  # 3 oder weniger Plätze → wird nicht gebucht
        else:
        ...
        for ev in events_to_process:
            ...
            if ev["status"] == "pending":
                if is_excluded:
                    ...
                    # Normale Buchung für 4 Personen
                    import asyncio
                    ...
                    if success:
                        ...
                        event.capacity = 4
                        db.commit()
                        logger.info(f"Event '{ev['title']}' am {ev['date'].strftime('%d.%m.')} SOFORT gebucht für 4 Personen")
